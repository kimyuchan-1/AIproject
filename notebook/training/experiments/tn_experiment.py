"""
TN 하이퍼파라미터 그리드 탐색 실험
====================================
타겟: TN_VU (총질소, 30분 평균)
DATA_LEAKAGE_CONFIG: safe_process_features = [FLUX_VU, TOC_VU, SS_VU, TP_VU, PH_VU]
...
TIME_COL = "SYS_TIME"
RANDOM_SEED = 42
CURRENT_BEST_R2 = 0.9011  # 현재 최고 (노트북 탐색 결과)
...
PHASE1_GRID = {
    ...
    "use_attention": [False],
    "weight_decay": [0],
    "split_type": ["lc"],  # TN도 LC_SPLIT_RATIOS (80/10/10)
    "shuffle": [True],
}
...
RESULTS_FILE = "tn_experiment_results.csv"
...
def run_phase2(X, y, best_config, top_n=10):
    ...
    phase2_grid = {
        ...
        "split_type": ["lc"],  # TN는 LC_SPLIT_RATIOS
        "shuffle": [True],
    }
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="TN Hyperparameter Search")
