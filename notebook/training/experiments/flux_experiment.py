"""
FLUX 하이퍼파라미터 그리드 탐색 실험
====================================
타겟: FLUX_VU (누적 유량 차분값, 30분 합산)
DATA_LEAKAGE_CONFIG: safe_process_features = [TOC_VU, SS_VU, TN_VU, TP_VU, PH_VU]

실험 전략:
  - Phase 1: 핵심 파라미터 탐색 (hidden 128~512, layers 1~4, lr 2e-4~2e-3) - 64 조합
  - Phase 2: 최적 핵심 파라미터 기반 보조 파라미터 미세 조정
...
TIME_COL = "SYS_TIME"
RANDOM_SEED = 42
CURRENT_BEST_R2 = 0.6296  # 현재 최고 (노트북 탐색 결과)
...
# ====== Phase 1: 핵심 파라미터 탐색 ======
# TP/TOC 실험 인사이트 반영:
#   - layers=1이 압도적으로 우수 (TP Top10 중 8개, TOC Top10 중 8개가 layers=1)
#   - lr=2e-4는 너무 느려 최적 범위 밖 -> 5e-4~2e-3 탐색
#   - hidden=128은 성능 한계 있음 -> 256~512 탐색
# 총 18 조합 (64 -> 18로 효율화)
PHASE1_GRID = {
    ...
    "use_attention": [False],
    "weight_decay": [0],
    "split_type": ["lc"],  # FLUX도 LC_SPLIT_RATIOS (80/10/10)
    "shuffle": [True],
}
...
def preprocess_data(dfs):
    ...
    # FLUX: stability_ratio=0.3 (TP의 0.2와 TOC의 0.4 중간)
    sr = 0.3
    recommended_idx = wf_selector.get_recommended_features(stability_ratio=sr)
    print(f"\nFLUX 모드: stability_ratio={sr}")
...
RESULTS_FILE = "flux_experiment_results.csv"
...
def run_phase2(X, y, best_config, top_n=10):
    ...
    # TP/TOC 인사이트: attention=True는 항상 성능 하락 -> False만 사용
    ...
    phase2_grid = {
        ...
        "split_type": ["lc"],  # FLUX는 LC_SPLIT_RATIOS
        "shuffle": [True],
    }
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="FLUX Hyperparameter Search")
    ...
    parser.add_argument("--best-lr", type=float, default=5e-4)
    args = parser.parse_args()

    print("=" * 70)
    print("FLUX 하이퍼파라미터 그리드 탐색")
