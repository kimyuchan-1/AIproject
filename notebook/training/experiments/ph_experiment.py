"""
PH 하이퍼파라미터 그리드 탐색 실험
====================================
타겟: PH_VU (pH, 30분 평균)
DATA_LEAKAGE_CONFIG: safe_process_features = [FLUX_VU, TOC_VU, SS_VU, TN_VU, TP_VU]
...
TIME_COL = "SYS_TIME"
RANDOM_SEED = 42
CURRENT_BEST_R2 = 0.8432  # 현재 최고 (노트북 탐색 결과)
...
# ====== Phase 1: 핵심 파라미터 탐색 ======
# 누적 인사이트:
#   - TP/TOC: layers=1이 압도적, FLUX/SS: layers=2가 약간 우수
...
PHASE1_GRID = {
    ...
    "use_attention": [False],
    "weight_decay": [0],
    "split_type": ["lc"],  # PH도 LC_SPLIT_RATIOS (80/10/10)
    "shuffle": [True],
}
...
def preprocess_data(dfs):
    ...
    # FLUX_VU 차분 처리 (PH의 safe 특성으로 포함되므로 diff 형태가 더 의미 있음)
...
RESULTS_FILE = "ph_experiment_results.csv"
...
def run_phase2(X, y, best_config, top_n=10):
    ...
    phase2_grid = {
        ...
        "split_type": ["lc"],  # PH는 LC_SPLIT_RATIOS
        "shuffle": [True],
    }
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PH Hyperparameter Search")
